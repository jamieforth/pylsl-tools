...
class OscController(ControlSender):
    ...
    def counter(self, start_time_lsl, sf, stop_event, latency):
        ...
        while not stop_event.is_set():
            ...
            print(elapsed_time)
            self.outlet.push_sample([json.dumps({"seconds": elapsed_time})])
    ...
    def run(self):
        ...
        info = self.make_stream_info(
            self.name, self.content_type, self.source_id, self.manufacturer
        )
            ...
            while not self.is_stopped():
                ...
                if state == "start":
                    ...
                    self.outlet.push_sample(
                        [json.dumps({"state": self.control_states.START.name})],
                    ...
                    msg = osc_message_builder.OscMessageBuilder(
                        address="/lsl/record/start"
                    )
                    bundle.add_content(msg.build())
                    sclang.send(bundle.build())
                    self.counter_thread = Thread(
                        target=self.counter,
                        args=(lsl_time, 1 / 5, self.stop_event, self.latency),
                    )
                    self.counter_thread.start()
                elif state == "pause":
                    unixtime = time.time()
                    self.outlet.push_sample(
                        [json.dumps({"state": self.control_states.PAUSE.name})]
                    )
                    bundle = osc_bundle_builder.OscBundleBuilder(
                        unixtime + self.latency
                    )
                    msg = osc_message_builder.OscMessageBuilder(
                        address="/lsl/record/pause"
                    )
                    bundle.add_content(msg.build())
                    sclang.send(bundle.build())
                elif state == "stop":
...
def main():
    ...
    parser.add_argument("--name", default="audio", help="Name of the control stream.")
    parser.add_argument(
        "--content-type", default="marker", help="Content type for marker stream."
    )
    parser.add_argument(
        "--source-id", default="", help="Unique identifier for stream source."
    )
    parser.add_argument("--sc-ip", default="127.0.0.1", help="The ip of the OSC server")
    parser.add_argument(
        "--sclang-port",
        type=int,
        default=57120,
