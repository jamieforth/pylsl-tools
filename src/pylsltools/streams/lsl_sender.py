...
class LSLSender:
    ...
    def __init__(
        ...
        if desc:
            self.info = self.make_stream_info_with_desc(
                name,
                content_type,
                channel_count,
                nominal_srate,
            ...
            if isinstance(channel_labels, list):
                if len(channel_labels) == channel_count:
                    ...
                    print(
                        "{channel_count} channel labels required, {len(channel_labels)} provided."
                    )
            ...
            if isinstance(channel_types, list):
                if len(channel_types) == channel_count:
                    ...
                    print(
                        "{channel_count} channel types required, {len(channel_types)} provided."
                    )
                    channel_types = "misc"
            ...
            if isinstance(channel_units, list):
                if len(channel_units) == channel_count:
                    ...
                    print(
                        "{channel_count} channel units required, {len(channel_units)} provided."
                    )
                    channel_units = None
            if isinstance(channel_units, str):
                channel_units = [channel_units] * channel_count

            self.info = self.make_stream_info(
                name,
                content_type,
                channel_count,
                nominal_srate,
    ...
    def make_stream_info(
        ...
        info = StreamInfo(
            name, content_type, channel_count, nominal_srate, channel_format, source_id
        )
    ...
    def create_outlet(self, chunk_size, max_buffered):
        self.outlet = StreamOutlet(
            self.info, chunk_size=chunk_size, max_buffered=max_buffered
        )
